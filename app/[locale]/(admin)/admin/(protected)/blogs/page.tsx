    ...
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-400">{blogs.length} post(s)</p>
        </div>
              ...
              <tr key={b.id} className="hover:bg-white/5">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-3">
                    <Link href={`/admin/blogs/${b.id}/edit`} className="text-xs font-medium text-gold-400 hover:text-gold-300">Edit</Link>
                    <DeleteButton action={deleteBlog} id={b.id} confirmMessage={`Delete post "${b.titleEn}"?`} />
                  </div>
                </td>
              </tr>
            ))}
            {blogs.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-10 text-center text-ink-500">
                  No posts yet. Click <span className="text-gold-400">New post</span> to write one.
                </td>
              </tr>
            ) : null}
          </tbody>
